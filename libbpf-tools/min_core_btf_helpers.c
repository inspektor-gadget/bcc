/* SPDX-License-Identifier: (LGPL-2.1 OR BSD-2-Clause) */

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/utsname.h>
#include <zlib.h>

#include "trace_helpers.h"
#include "min_core_btfs.h"

#define FIELD_LEN 65

struct os_info {
	char id[FIELD_LEN];
	char version[FIELD_LEN];
	char arch[FIELD_LEN];
	char kernel_release[FIELD_LEN];
};

const char id_prefix[] = "ID=";
const char version_id_prefix[] = "VERSION_ID=";

static struct os_info get_os_info() {
	struct os_info info = {};
	struct utsname u;
	size_t len = 0;
	ssize_t read;
	char *line=NULL;
	FILE *f;

	uname(&u);

	strncpy(info.kernel_release, u.release, FIELD_LEN);
	strncpy(info.arch, u.machine, FIELD_LEN);

	f = fopen("/etc/os-release", "r");
	if (!f)
		return info;

	while ((read = getline(&line, &len, f)) != -1) {
		if (strncmp(id_prefix, line, strlen(id_prefix)) == 0) {
			strncpy(info.id, strchr(line, '=') + 1, FIELD_LEN-1);
			info.id[strlen(info.id)-1] = '\0';
		} else if (strncmp(version_id_prefix, line, strlen(version_id_prefix)) == 0) {
			strncpy(info.version, strchr(line, '=') + 2, FIELD_LEN-1);
			info.version[strlen(info.version)-2] = '\0';
		}
	}

	free(line);
	fclose(f);

	return info;
}

#define INITIAL_BUF_SIZE (1024*1024*4) // 4MB

/* adapted from https://zlib.net/zlib_how.html */
static int
inflate_gz(unsigned char *src, int src_size, unsigned char **dst, int *dst_size) {
	size_t size = INITIAL_BUF_SIZE;
	size_t next_size = size;
	z_stream strm;
	int ret;

	*dst = malloc(size);
	if (!*dst)
		return -1;

	strm.zalloc = Z_NULL;
	strm.zfree = Z_NULL;
	strm.opaque = Z_NULL;
	strm.avail_in = 0;
	strm.next_in = Z_NULL;

	ret = inflateInit2(&strm, 16+MAX_WBITS);
	if (ret != Z_OK)
		return ret;

	strm.next_in = src;
	strm.avail_in = src_size;

	/* run inflate() on input until it returns Z_STREAM_END */
	do {
		strm.next_out = *dst + strm.total_out;
		strm.avail_out = next_size;
		ret = inflate(&strm, Z_NO_FLUSH);
		if (ret != Z_OK && ret != Z_STREAM_END)
			goto out;
		/* we need more space */
		if (strm.avail_out == 0) {
			next_size = size;
			size *= 2;
			*dst = realloc(*dst, size);
		}
	} while (ret != Z_STREAM_END);

	*dst_size = strm.total_out;

out:
	/* clean up and return */
	inflateEnd(&strm);
	return ret == Z_STREAM_END ? Z_OK : Z_DATA_ERROR;
}

/* tar header from https://github.com/tklauser/libtar/blob/v1.2.20/lib/libtar.h#L39-L60 */
struct tar_header {
	char name[100];
	char mode[8];
	char uid[8];
	char gid[8];
	char size[12];
	char mtime[12];
	char chksum[8];
	char typeflag;
	char linkname[100];
	char magic[6];
	char version[2];
	char uname[32];
	char gname[32];
	char devmajor[8];
	char devminor[8];
	char prefix[155];
	char padding[12];
};

char *tar_file_start(struct tar_header *tar, const char *name, int *length) {
	while (tar->name[0]) {
		sscanf(tar->size, "%o", length);
		if (!strcmp(tar->name, name))
			return (char *)(tar+1);
		tar += 1 + (*length+511)/512;
	}
	return NULL;
}

int save_min_core_btf(char *template) {
	char name_fmt[] = ".output/min_core_btfs/%s/%s/%s/%s.btf";
	struct os_info info = get_os_info();
	unsigned char *dst_buf = NULL;
	char *file_start;
	int dst_size = 0;
	char name[100];
	FILE *dst = NULL;
	int ret;

	/* do nothing if the system provides BTF */
	if (vmlinux_btf_exists())
		return -1;

	ret = mkstemp(template);
	if (ret < 0)
		return ret;

	dst = fdopen(ret, "wb");
	if (!dst)
		return -1;

	ret = snprintf(name, sizeof(name), name_fmt, info.id, info.version,
		       info.arch, info.kernel_release);
	if (ret < 0 || ret == sizeof(name))
		goto out;

	ret = inflate_gz(_output_min_core_btfs_tar_gz, _output_min_core_btfs_tar_gz_len,
			 &dst_buf, &dst_size);
	if (ret != Z_OK)
		goto out;

	ret = 0;
	file_start = tar_file_start((struct tar_header *)dst_buf, name, &dst_size);
	if (!file_start) {
		ret = -1;
		goto out;
	}

	if (fwrite(file_start, 1, dst_size, dst) != dst_size) {
		ret = -1;
		goto out;
	}

out:
	fclose(dst);
	free(dst_buf);

	return ret;
}
